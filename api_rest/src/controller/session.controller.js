...
import { createHash } from '../utils/utils.js';oder
...
export default class SessionController {
    ...
    postLogin = async(req, res, next) => {
        ...
        try {
            ...
            return res.cookie('UlisesCookieToken', token, {maxAge: 1000*60*24, httpOnly: true}).send({status: "Ok", message: "Logged in", payload: user});
        } catch (error) {
            next(error);
        }
    }
    ...
    postLogout = async(req, res, next) => {
        ...
        try {
            res.clearCookie("UlisesCookieToken")
    ...
    postRecover = async(req, res, next) => {
        ...
        try {
            ...
            try {
                transport.sendMail({from: 'ulises.roa07@gmail.com',
                to: email,
    ...
    postSwapUserClass = async(req, res, next) => {
        try {
            ...
            if (dbUser.role == "user") {
                ...
                let token = jwt.sign({user}, config.jwtKey, {expiresIn: "24h"});
                if (result.acknowledged) return res.cookie('UlisesCookieToken', token, {maxAge: 1000*60*24, httpOnly: true}).send({status: "Ok", message: "Rol actualizado"});
