...
export default class SessionController {
    ...
    postLogin = async(req, res, next) => {
        ...
        try {
            ...
            return res.cookie('CookieToken', token, {maxAge: 1000*60*24, httpOnly: true}).send({status: "Ok", message: "Logged in", payload: user});
        } catch (error) {
            next(error);
        }
    }
    ...
    postRecover = async(req, res, next) => {
        ...
        try {
            ...
            try {
                ...
                html: `
                <div style="background-color: black; color: green; display: flex; flex-direction: column; justify-content: center;  align-items: center;">
                <h1>Para reestablecer tu contraseña haz click <a href="https://ecommerce.up.railway.app/recoverLanding/${token}">aqui</a></h1>
                </div>
                `});
            } catch (error) {
    ...
    postSwapUserClass = async(req, res, next) => {
        try {
            ...
            if (dbUser.role == "user") {
                ...
                let token = jwt.sign({user}, config.jwtKey, {expiresIn: "24h"});
                if (result.acknowledged) return res.cookie('CookieToken', token, {maxAge: 1000*60*24, httpOnly: true}).send({status: "Ok", message: "Rol actualizado"});
